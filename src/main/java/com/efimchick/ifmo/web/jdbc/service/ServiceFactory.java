...
public class ServiceFactory {
    ...
    public EmployeeService employeeService(){
        return new EmployeeService() {
            ...
            String departmentSQLStatement = "SELECT * FROM EMPLOYEE WHERE DEPARTMENT = ";

            @Override
            public List<Employee> getByDepartmentSortByHireDate(Department department, Paging paging) {
                return getRequestedPage(paging, departmentSQLStatement + department.getId() + "ORDER BY HIREDATE");
            }

            @Override
            public List<Employee> getByDepartmentSortBySalary(Department department, Paging paging) {
                return getRequestedPage(paging, departmentSQLStatement + department.getId() + " ORDER BY SALARY");
            }

            @Override
            public List<Employee> getByDepartmentSortByLastname(Department department, Paging paging) {
                return getRequestedPage(paging, departmentSQLStatement + department.getId() + " ORDER BY LASTNAME");
            }

            String managerSQLStatement = "SELECT * FROM EMPLOYEE WHERE MANAGER = ";

            @Override
            public List<Employee> getByManagerSortByLastname(Employee manager, Paging paging) {
                return getRequestedPage(paging, managerSQLStatement + manager.getId() + " ORDER BY LASTNAME");
            }

            @Override
            public List<Employee> getByManagerSortByHireDate(Employee manager, Paging paging) {
                return getRequestedPage(paging, managerSQLStatement + manager.getId() + " ORDER BY HIREDATE");
            }

            @Override
            public List<Employee> getByManagerSortBySalary(Employee manager, Paging paging) {
                return getRequestedPage(paging, managerSQLStatement + manager.getId() + " ORDER BY SALARY");
            }
